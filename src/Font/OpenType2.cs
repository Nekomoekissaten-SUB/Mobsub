...
public class OpenType2 : IParseFonts
{
    private static readonly ushort curLanguageId = GetCurrentLanguageId();
    ...
    private static void GetFontsInfo(FileInfo[] fileInfos, ICollection<FontFaceInfoOpenType> infos)
    {
        foreach (var fileInfo in fileInfos)
        {
            ...
            using var fp = new OpenTypeFileParse2(fileInfo.FullName);
            if (!fp.Open()) { continue; }
    ...
    private static ushort GetCurrentLanguageId()
    {
        var lcid = (ushort)CultureInfo.CurrentCulture.LCID;
        return lcid == 0 ? (ushort)0x0409 : lcid; // fallback en-US
    }
}
