...
internal static class PerspectiveApplyTagsFromClipQuadHandler
{
    private const string ClassDialogue = "dialogue";

    public static BridgeHandlerResult Handle(PerspectiveApplyTagsFromClipQuadCall call, List<string> logs)
    {
        ...
        Span<Vector2> clipQuad = stackalloc Vector2[4];
        foreach (var line in lines)
        {
            ...
            {
                logs.Add($"missing_size line={line.Index}: need line.width/line.height or args.width/height.");
                continue;
            }

            if (!AssClipQuadExtractor.TryGetQuadFromFirstOverrideBlock(read, clipQuad, out var error))
            {
                logs.Add($"clip_quad_missing line={line.Index}: {error}");
                continue;
            }
