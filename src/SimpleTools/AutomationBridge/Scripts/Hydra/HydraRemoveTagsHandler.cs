...
internal static class HydraRemoveTagsHandler
{
    private const string ClassDialogue = "dialogue";
    private const int MaxTransformDepth = 8;

    public static BridgeHandlerResult Handle(HydraRemoveTagsCall call, List<string> logs)
    {
        ...
        HydraTagScope scope = HydraTagSet.NormalizeScope(call.Args.Scope);
        ...
        foreach (var line in lines)
        {
            ...
            for (int s = 0; s < segments.Length; s++)
            {
                ...
                // "First block" means the leading tag block that starts at byte 0.
                if (scope == HydraTagScope.FirstBlock)
                {
                    if (s != 0)
                        break;

                    var (start0, _) = GetRangeOffsets(seg.LineRange, utf8.Length);
                    if (start0 != 0)
                        break;
                }
                ...
                byte[]? newPayload = HydraPayloadRewriter.RemoveTags(payload, tagSet, depth: 0, maxDepth: MaxTransformDepth);
            ...
            newTextUtf8 = AssOverrideTagRewriter.RemoveEmptyOverrideBlocks(newTextUtf8);
